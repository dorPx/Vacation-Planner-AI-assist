...
function priceDollars(n: number) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="bg-gradient-to-r from-blue-600 to-blue-500 text-white px-5 py-3 flex items-center justify-between">
        ...
        <span className="text-sm font-medium">Est. {priceDollars(day.estimated_cost)}</span>
      </div>
          ...
          <div>
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                <p className="font-semibold text-slate-800 text-sm">{priceDollars(day.hotel.price_per_night)}</p>
              ...
              {day.activities.map((a) => (
                <li key={a.id} className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{a.duration_hours}h · {a.price > 0 ? priceDollars(a.price) : 'Free'}</p>
                  </div>
                </li>
              ))}
            </ul>
    ...
    <div className="space-y-4">
      <div className="bg-gradient-to-r from-slate-800 to-slate-700 text-white rounded-xl p-5 flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className="text-2xl font-bold text-green-400">${itinerary.total_cost.toLocaleString()}</p>
        </div>
      </div>
